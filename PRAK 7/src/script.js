...
function tampilkanDaftarBuku() {
    ...
    daftarBuku.forEach((buku, index) => {
        ...
        divBuku.innerHTML = `
            ...
            <button class="w mt-2 bg-blue-500 hover:bg-blue-600 text-white p-2 rounded transition" onclick="tambahKeFavorit(${index})">Tambah ke Favorit</button>
        `;
...
function tampilkanBukuFavorit() {
    ...
    bukuFavorit.forEach((buku, index) => {
        ...
        divBuku.innerHTML = `
            <p>${buku.tampilkanInfo()}</p>
            <button class="w mt-2 bg-red-500 hover:bg-red-600 text-white p-2 rounded" onclick="hapusDariFavorit(${index})">Hapus</button>
        `;
